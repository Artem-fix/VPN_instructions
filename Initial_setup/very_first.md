...
---
Тут я рекламировать хостеров не буду, тем более они постояно меняются, где-то становится дорого, где-то запрашивают личные данные, паспорта и так далее, по этому просто расскажу про китерии выбора сервера, а вы уже сами определитесь с выбором.
...
Идеальным способом оплаты будет оплата анонимной криптовалютой (Altcoin) Monero/Dash/Zcash (но далеко не все провайдеры имеют такой способ оплаты, вот список тех, [кто поддерживает](https://www.bitcoin-vps.com/)).
...
- &ensp; Географическое расположение сервера: один из самых важных пунктов. При аренде сервера можно выбрать страну, в которой он будет располагаться. Очевидно, что сервер не должен располагаться в России, ведь тогда ни к каким заблокированным сайтам/сервисам мы доступ не получим. Какую страну выбрать? Зависит от ваших потребностей, но я рекомендую страны, удовлетворяющие всем или большинству условий ниже:
    ...
    - &emsp; Страна имеет связи с другими странами, в том числе с РФ по глобальным [волоконно-оптическим кабелям Интернета](https://www.submarinecablemap.com/).
...
2. &ensp; Проверим наличие IP-адреса в чёрных списках Роскомнадзора ([тут](https://blocklist.rkn.gov.ru/) или [тут](https://2ip.ru/rkn-blacklist/)).
3. &ensp; Теперь проверим наличие IP-адреса в спам-базах ([тут](https://2ip.ru/spam/)). Одна-две записи - это относительно нормально, а вот большее кол-во вызывает вопросы.
...
Если вы в будующем собираетесь настраивать ВПН протокол Amnezia 2, то вам нужно сначала [настроить этот протокол](../Setting_VPN/AmneziaWG_2.0/AmneziaWG.md), а уже потом перейти к шагу 3!!!
...
2. &emsp; Заходим в настройки ssh-соединения с помощью редактора nano (всё делаем от суперпользователя):
```
...
3. &emsp; Выполняем перезагрузку ssh командой:
```
systemctl restart sshd
```
4. &emsp; Теперь подключение к вашему серверу будет выглядеть, например, так (имя пользователя, ip-адрес и порт указываете ваш):
```
...
5. &emsp; И напоследок крайне желательно использовать подключение по ssh не по паролю, а с помощью ключей, ведь подобрать ваш 20-значный пароль проще, чем очень длинный и сложный закрытый-ключ. Также настройка ssh-ключей позволит вам проще подключаться к серверу.
...
## Шаг 4. Выбор ВПН протокола

Пока что я не знаю что тут написать так как сам не определился какой лучше по соотношению скорость/бехопасность/обход ТСПУ, по этому вы можете сами погуглить и выбрать.

[Настройка olcRTC](/Setting_VPN/Olcrtc.md)

[Настройка Vless](/Setting_VPN/Vless.md)
